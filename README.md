...
- **Wear OS companion app** for Wear OS 4+ uses native Kotlin and Compose and requires the phone app.
  ...
  breastfeeding, automatic sleep, pumping, and tummy-time timers. Breastfeeding timers support side
  switching, while pumping timers support the left, right, or both sides and record volume when
  stopped. The watch also logs formula and breast-milk bottles from 0 to 500 ml. The sleep section shows the last
  completed sleep and current awake or wake-window timing. It also shows a phone-confirmation notice
  when needed.
