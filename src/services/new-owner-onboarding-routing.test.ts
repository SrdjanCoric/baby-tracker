...
describe("getNewOwnerPreviewDestination", () => {
  it.each([
  ...
  ] as const)("routes %s to its isolated restoration destination", (state, route, ownerLeaf) => {
    expect(getNewOwnerPreviewDestination(state as NewOwnerOnboardingState)).toEqual({
      route,
      ownerLeaf,
    });
  });
