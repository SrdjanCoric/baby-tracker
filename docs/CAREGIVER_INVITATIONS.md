...
The SQL test covers owner authorization, email normalization, replacement, multiple pending invitations, expiry, revocation, one-time consumption, verified-email matching, legacy-code rejection, own-household and shared-household rejection, destructive solo-data deletion, the preserved RPC signature, and server rate limiting. The caregiver Maestro fixture uses only local Supabase and resets the invited account to a solo household with `Solo Baby` before creating `E2EJ-2345`.

The network command is the authoritative interruption proof. It refuses an existing Metro process, then starts Metro itself for the current checkout with local Supabase configuration before changing fixtures. It stops the local API after the destructive confirmation appears, submits the join, observes Retry, restarts the app while the API remains offline, restores the API, and retries through the same production UI. The final SQL assertion requires one consumed invitation, one target-household profile assignment, both target babies, removal of the confirmed solo household, and preservation of unrelated fixture rows. Each run resets these dedicated fixtures first, so rerunning requires no checkpoint edits. The successful fixture state and timestamped logs remain available under `e2e/artifacts/onboarding-network/` for inspection; the next run resets them. The API restoration trap runs on success, command failure, `SIGINT`, and `SIGTERM`. If the command fails, inspect the reported directory's phase log, `failure-summary.log`, and `database-diagnostics.log`.
