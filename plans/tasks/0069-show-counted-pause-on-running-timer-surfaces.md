...
- **`buildOngoingSleepEntry` in `src/utils/ongoing-sleep.ts`** — it currently shifts the entry's start
  ...
  one. This entry feeds the Statistics sleep screens' Day view blocks and total, the Timeline daily
  summary for sleep, and `SleepPredictionCard`.
- **`DashboardCard` and `CompactActivityRow`** — both take `timerStartTime`, `timerPausedAt`, and
  ...
  it from the four call sites on the dashboard and from the components' memo comparators.
- **The four activity screens** — `app/feeding/index.tsx`, `app/sleep/index.tsx`,
  ...
  for sleep and tummy time, an elapsed-minutes figure that drives a duration alert. Both figures follow
  the new rule, so an alert fires on total elapsed rather than on active-only time.
...
- [ ] Rewrite `buildOngoingSleepEntry` so the entry starts at the timer's real start, ends at
...
- [ ] Update the sleep surfaces that consume the ongoing entry so the Day view block, the Day view
      total, the Timeline daily summary, and `SleepPredictionCard` all report the frozen value during a
      pause and the counted value after a resume.
- [ ] Apply the rule to the `DashboardCard` and `CompactActivityRow` tickers for all four types, and
      resolve the now-unused `timerTotalPausedMs` prop consistently across the components, the
      dashboard call sites, and the memo comparators.
- [ ] Apply the rule to the displayed elapsed value on all four activity screens, and to the
      elapsed-minutes figure driving the sleep and tummy time duration alerts.
- [ ] Apply the rule to the widget payload in `widget-context`: real start for `effectiveStart`, and
      `pausedAt - startedAt` for the paused `accumulatedSeconds`, for each of the four types.
- [ ] Pass total elapsed to `pauseTimerLiveActivity` and `resumeTimerLiveActivity` from all four
      contexts, and remove the `totalPausedMs` shift from the lifecycle module's Live Activity restart.
- [ ] Per type, add coverage that a running timer's readout stops growing while a pause is open and
...
- [ ] Add coverage that a running timer's displayed elapsed equals the `durationSeconds` the record
...
- [ ] For each of the four types, the dashboard card, the compact activity row, and the activity screen
      stop advancing while the timer is paused and read `pausedAt - startedAt`.
- [ ] For each of the four types, after a resume those surfaces read `now - startedAt`, including the
      resumed pause span.
- [ ] The Statistics Day view block and total, the Timeline daily summary, and `SleepPredictionCard`
      report a running paused sleep frozen at `pausedAt` and a resumed one counting the paused span.
- [ ] The widget payload carries the real start for every running timer and, while paused,
      `accumulatedSeconds` equal to `pausedAt - startedAt`.
- [ ] The Live Activity pause, resume, and restart paths receive total elapsed, with no
      `totalPausedMs` shift left anywhere.
- [ ] A running timer's displayed elapsed equals the `durationSeconds` a stop at that instant would
      record, for every type, both paused and resumed.
- [ ] No native target, control, label, or record-writing path is changed by this task.
