...
- Added server-side operation acknowledgements and a user-bound `merge_record` overload in `055_idempotent_owned_sync_operations.sql`. Replays with the same operation ID apply once, including concurrent calls. The legacy authenticated overload remains available during the mobile rollout so applying the migration does not break installed app versions; it can be revoked after those versions are no longer supported.
...
- Verification passed: `npm run test:sync` (20 files, 243 tests); `npm run test:unit` (98 files, 2,237 tests); `npm run test:component -- --runInBand` (44 suites, 547 tests); `npm run test:security` (8 files, 89 tests); `npm run test:sql` (26 vectors, 45 merge assertions, tombstone and concurrency checks); `npm run typecheck`; `npm run lint`; and `git diff --check`.
...
- Merged through PR #116 as `06ab68a289e6569dd72afdf1d3553f85509509d5` on July 15, 2026.
