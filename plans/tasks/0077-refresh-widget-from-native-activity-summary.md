...
- [ ] Write failing local SQL contract and authorization tests for the versioned selected-baby
...
- [ ] Add the authenticated read-only RPC migration with `SECURITY INVOKER`, fully qualified
...
- [ ] Implement every currently displayed Widget summary field with the shipped day-boundary,
...
- [ ] Prove the three timer-completion read states locally: lock only; matching completed activity
...
- [ ] Add shared legacy/versioned JSON fixtures that TypeScript and the Widget Swift decoder both
      accept, including additive unknown metadata and the singular-timer fallback.
- [ ] Add the Widget refresh/store coordinator test-first, including full replacement, in-flight
...
- [ ] Route every Widget timeline provider and post-action reconciliation through the complete
      summary coordinator, and remove timer-only network results from every authoritative commit path.
- [ ] Preserve existing push registration, timer-change push triggers, scheduled timeline cadence,
      local elapsed rendering, and legacy cache decoding without adding short-interval polling.
- [ ] Measure the RPC payload and run local `EXPLAIN (ANALYZE, BUFFERS)` against representative
...
- [ ] Extend the local two-caregiver sleep gate so caregiver A caches a running sleep, caregiver B
...
- [ ] Run focused SQL, security, payload, native-source, component, and two-caregiver checks plus the
      repository's relevant canonical validation.

## Human checkpoints

- [ ] [confirm-security] Approve the final RPC signature, `SECURITY INVOKER` choice, grants, caller-
...
- [ ] One authenticated selected-baby RPC returns every field the Widget currently displays, as a
      compact summary rather than raw activity history or multiple babies.
- [ ] Same-household access succeeds; anonymous, outside-household, deleted-baby, wrong-baby, and
      tombstoned data cannot leak into a response.
- [ ] A timer and its dependent summaries always describe one coherent database state, including a
      refresh between activity persistence and lock release.
- [ ] A remote sleep stop updates timer absence, `lastSleepEndedAt`, awake duration inputs, duration,
      type, current-day minutes, nap count, and wake-window state together without opening the app.
- [ ] Every failed, malformed, stale, or wrong-scope refresh leaves the previous complete cache
      unchanged and retryable.
- [ ] Widget refresh remains push/timeline/action driven with no short-interval polling, and concurrent
      triggers for one baby coalesce.
- [ ] Old installed clients remain operational, and the new client reads legacy unversioned caches
      until its first valid versioned snapshot.
- [ ] Local query-plan and payload measurements show bounded selected-baby work; any new index is
      justified by measured evidence.
- [ ] No implementation or verification step reads from or mutates production.
