...
- [ ] Replace the household-wide join implementation behind the existing RPC signature with atomic verified-email matching, single-use redemption, and an explicit post-release enforcement switch.
...
- [ ] Legacy household-wide codes remain usable before app deployment and no longer authorize joins after the release owner enables email enforcement.
